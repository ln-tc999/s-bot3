...
export function PaperDesignBackground({
  ...
  // Derived colors and speeds. s-bot3 brand: forest → mint (BOT green)
  const config = useMemo(() => {
    ...
    if (isDark) {
      return {
        back: "#00000000",
        front: mix("#10A37F", "#A4EF6B", t * 0.35), // BOT brand green
        bg: "#07130F",
        speed: 0.28 + t * 0.35,
        px: Math.round(2 + t * 2),
        scale: 1.05 + t * 0.15,
        glow: "radial-gradient(60% 40% at 50% 40%, rgba(16,163,127,0.18), transparent 70%)",
      };
    } else {
      return {
        back: "#00000000",
        front: mix("#10A37F", "#A4EF6B", t * 0.35), // light mode, same BOT green
        bg: "#F4FAF7",
        speed: 0.22 + t * 0.28,
        px: Math.round(2 + t * 2),
        scale: 1.03 + t * 0.12,
        glow: "radial-gradient(60% 40% at 50% 40%, rgba(16,163,127,0.12), transparent 70%)",
      };
    }
  }, [isDark, intensity]);
